...
export const CONTRACT_ADDRESS =
  "CCNVR62ROF3MFU4MYZWGIEPRVLBGWQVVVFYNPW44TVVHXA4AYNJP7ARG";
...
/**
 * Create a new product in the supply chain. PERMISSIONLESS: Anyone can create.
 * Calls: create_product(id: String, name: String, origin: String)
 */
export async function createProduct(
  ...
  return callContract(
    "create_product",
    [toScValString(id), toScValString(name), toScValString(origin)],
    caller,
    true
  );
}
 ...
 * Calls: update_status(id: String, status: String)
 */
export async function updateStatus(
  ...
  return callContract(
    "update_status",
    [toScValString(id), toScValString(status)],
    caller,
    true
  );
}
 ...
 * Calls: get_product(id: String)
 * Returns: { id, name, origin, status, timestamp } or error
 */
...
/**
 * Get product status history (read-only).
 * Calls: get_history(id: String)
 * Returns: Array of { status, timestamp } entries
 */
